...
public class StorageProxy implements StorageProxyMBean
{
    ...
    private static List<Row> fetchRowsUsingRein(List<ReadCommand> initialCommands, ConsistencyLevel consistencyLevel)
    throws UnavailableException, ReadFailureException, ReadTimeoutException
    {
        ...
        do
        {
            List<AbstractReadMultiExecutor> readExecutors = new ArrayList<AbstractReadMultiExecutor>();
            HashMap<InetAddress, ArrayList<ByteBuffer>> replicaGroupReqs = new HashMap<InetAddress, ArrayList<ByteBuffer>>();
            ...
            {
                ReadCommand command = initialCommands.get(i);
                Keyspace keyspace = Keyspace.open(command.ksName);
                List<InetAddress> allReplicas = StorageProxy.getLiveSortedEndpoints(keyspace, command.key);
                ReadRepairDecision repairDecision = ReadRepairDecision.NONE;
                List<InetAddress> targetReplicas = consistencyLevel.filterForQuery(keyspace, allReplicas, repairDecision);

                //exec.executeAsync();

                InetAddress rg = targetReplicas.get(0);
                if(!replicaGroupReqs.containsKey(rg))
                    replicaGroupReqs.put(rg, new ArrayList<ByteBuffer>());

                replicaGroupReqs.get(rg).add(command.key);
            ...
            ReadCommand command = initialCommands.get(0);
            HashMap<InetAddress, ReadMultiCommand> rms = new HashMap<InetAddress, ReadMultiCommand>();

            //String opsetsStr = " ";
            for (InetAddress rg: replicaGroupReqs.keySet())
            {
                ReadMultiCommand rm = ReadMultiCommand.create(command.ksName, replicaGroupReqs.get(rg), command.getColumnFamilyName(), command.timestamp, command.filter().cloneShallow());
                rm.setBatchSize(rm.keycount);
                rms.put(rg, rm);
                //opsetsStr += (Integer.toString(rm.keycount) + " ");
            }

            //logger.info("Opset sizes:{}",opsetsStr);

            logger.trace("[REIN] maxReqCount for all replica groups: " + maxReqCount);
            ...
            if((DatabaseDescriptor.getPriorityStrategy().equals("hybrid") || DatabaseDescriptor.getPriorityStrategy().equals("SBF"))
               && DatabaseDescriptor.getQueueType().equals("multiqueue") && DatabaseDescriptor.getReinEnabled())
            {
            ...
            for (InetAddress rg: replicaGroupReqs.keySet())
            {
                AbstractReadMultiExecutor exec = AbstractReadMultiExecutor.getReadExecutor(rms.get(rg), consistencyLevel);
                readExecutors.add(exec);

                if(DatabaseDescriptor.getQueueType().equals("multiqueue") && DatabaseDescriptor.getReinEnabled())
                {
                    ...
                    if((DatabaseDescriptor.getPriorityStrategy().equals("hybrid") || DatabaseDescriptor.getPriorityStrategy().equals("SDS"))
                       && maxReqCount>DatabaseDescriptor.getSDSThreshold())
                    {
                        if (maxReqRG != rg)
                        {
                ...
                else if(DatabaseDescriptor.getQueueType().equals("priority") && DatabaseDescriptor.getReinEnabled())
                {
                    if(DatabaseDescriptor.getPriorityStrategy().equals("equalMax"))
                    {
            ...
            for (AbstractReadMultiExecutor exec: readExecutors)
            {
                try
                {
                    List<Row> rgRows = exec.get();
                    if (rows.size() != 0)
                    {
                        //FIXME implement trimming for retries
                ...
                catch (DigestMismatchException ex)
                {
                    ...
                    //FIXME implement proper handling of digest mismatches
                }
            }
    ...
    static class LocalReadMultiRunnable extends DroppableRunnable implements PriorityProvider
    {
        private final ReadMultiCommand command;
        private final ReadCallback<ReadMultiResponse, List<Row>> handler;
        private final long start = System.nanoTime();

        LocalReadMultiRunnable(ReadMultiCommand command, ReadCallback<ReadMultiResponse, List<Row>> handler)
        {
            super(MessagingService.Verb.READ);
            this.command = command;
            this.handler = handler;
        ...
        protected void runMayThrow()
        {
            ...
            {
                Keyspace keyspace = Keyspace.open(command.ksName);
                List<Row> r = command.getRows(keyspace);
                ReadMultiResponse result = ReadMultiVerbHandler.getResponse(command, r);
                //logger.info("Result {}", result);
                long serviceTime = System.nanoTime() - start;
                MessagingService.instance().addLatency(FBUtilities.getBroadcastAddress(), TimeUnit.NANOSECONDS.toMillis(serviceTime));
                MessagingService.instance().updateLocalMetrics(serviceTime, command.getBatchSize(), command.getPriority());
                handler.response(result);
                //FIXME Should we decrement in the case of any errors?
                MessagingService.instance().getPendingRequestsCounter(FBUtilities.getBroadcastAddress()).decrementAndGet();
            }
