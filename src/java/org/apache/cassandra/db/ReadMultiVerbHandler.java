...
public class ReadMultiVerbHandler implements IVerbHandler<ReadMultiCommand>
{
    public void doVerb(MessageIn<ReadMultiCommand> message, int id)
    {
        ...
        ReadMultiCommand command = message.payload;
        Keyspace keyspace = Keyspace.open(command.ksName);
        List<Row> rows = command.getRows(keyspace);

        long serviceTimeInNanos = System.nanoTime() - startRead;
        int queueSize = counter.decrementAndGet();

        MessageOut<ReadMultiResponse> reply =
        new MessageOut<>(MessagingService.Verb.REQUEST_RESPONSE,
                         getResponse(command, rows),
                         ReadMultiResponse.serializer)
    ...
    public static ReadMultiResponse getResponse(ReadMultiCommand command, List<Row> rows)
    {
        if (command.isDigestQuery())
        {
            return new ReadMultiResponse(null);
        }
        else
        {
            return new ReadMultiResponse(rows);
        }
    }
}
