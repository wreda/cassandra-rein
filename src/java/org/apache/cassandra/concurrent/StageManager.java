...
import org.apache.cassandra.net.MultiConcurrentLinkedPriorityQueueDRR;
...
public class StageManager
{
    ...
    static
    {
        ...
        stages.put(Stage.READ, multiThreadedLowSignalStageRein(Stage.READ, getConcurrentReaders()));
    ...
    private static TracingAwareExecutorService multiThreadedLowSignalStageRein(Stage stage, int numThreads)
    {
        ...
        if(DatabaseDescriptor.getReinEnabled())
        {
            ...
            {
                q = new MultiConcurrentLinkedPriorityQueueDRR<AbstractTracingAwareExecutorService.FutureTask<?>>(DatabaseDescriptor.getQueueWeights());
            }
            else
            {
                q = new PriorityBlockingQueue<AbstractTracingAwareExecutorService.FutureTask<?>>(100, new PriorityComparator());
            }
        }
        else
        {
            q = new MultiConcurrentLinkedPriorityQueueDRR<AbstractTracingAwareExecutorService.FutureTask<?>>(Arrays.asList(100));
            DatabaseDescriptor.setInitialQueueRange(Integer.MAX_VALUE);
        }
